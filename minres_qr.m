function [x, residuals, setup_time, lanczos_times, qr_times, ...
    inversion_times, final_times, mat_mul1_times, mat_mul2_times] = minres_qr(A, b, optimize, reorthogonalize)
    ...
    for k = 1:size_A
        %fprintf("Iteration %g\n", k);
        %tic
            [V, T, prev_w] = iterative_lanczos(A, V, T, prev_w, b, k, reorthogonalize);
        %lanczos_times(k) = toc;
        %tic
