function [A_t, b_t, A, b] = generate_problem_matrices(...
...
%   solution to the original problem, a 0 needs to be appended to the final 
%   solution vector obtained, restoring the lost dimension.
%
% Inputs: 
%
...
% 
% - verbose (bool, defaults to false): if set to true, the function prints
...
%       version of the main problem matrix, missing its last row and column. 
%       It is composed by [D E'; E 0], where D is a diagonal
%       positive definite matrix, while E is the incidence matrix of the graph.
%
% - b_t ([(nodes-1)+edges x 1] real column vector): the truncated version of b, 
%       hence missing its last row. It is defined as [c; d]: 
%       c ([edges x 1]) is simply created by generating #edges random values in range [0, 1].
%       To comply the constraint imposed by the linear dependency of E rows, 
...
%       to its opposite (d_2 = -d_1) (this is done before the truncation).
%
% Optional outputs:
%
% - A ([nodes+edges x nodes+edges] real sparse matrix): same as A_t, but
%       including the truncated row and column.
%
% - b ([nodes+edges x 1] real column vector): same as b_t, but including
%       the truncated row.
%
